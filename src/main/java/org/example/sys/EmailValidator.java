...
public class EmailValidator {
    ...
    private static final Logger logger
            = LogManager.getLogger(EmailValidator.class);
    ...
    public static boolean isValid(String email) {
        ...
        if (!email.contains("@")) {
            logger.info("Brak znaku '@' w adresie e-mail: {}",
                    email);
            return false;
        }
        ...
        if (parts.length != 2) {
            logger.info("Nieprawidłowa liczba części " +
                    "po podziale '@': {}", email);
            return false;
        }
        ...
        logger.trace("Podzielono na lokalną część: '{}', domenę: '{}'",
                localPart, domainPart);

        if (localPart.isEmpty() || localPart.length() > 64) {
            logger.info("Nieprawidłowa długość lokalnej" +
                    " części (1–64 znaki): {}", localPart);
            return false;
        }

        if (domainPart.length() < 3 || domainPart.length() > 253) {
            logger.info("Nieprawidłowa długość" +
                    " domeny (3–253 znaki): {}", domainPart);
            return false;
        }
        ...
        if (domainPart.startsWith(".") || domainPart.endsWith(".")) {
            logger.info("Domena zaczyna się lub kończy kropką: {}",
                    domainPart);
            return false;
        }
        ...
        for (char c : localPart.toCharArray()) {
            if (allowedChars.indexOf(c) == -1) {
                logger.info("Znaleziono niedozwolony znak w lokalnej" +
                        " części: '{}', znak: '{}'", localPart, c);
                return false;
            }
        }
