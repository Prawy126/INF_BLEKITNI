...
public class TransactionProductRepository {
    ...
    private static final Logger logger
            = LogManager.getLogger(TransactionProductRepository.class);
    ...
    public void addTransactionProduct(TransactionProduct tp) {
        ...
        try {
            ...
            em.persist(tp);
            tx.commit();
            logger.info("addTransactionProduct() " +
                    "– zapisano: {}", tp);
        } catch (Exception e) {
            if (tx.isActive()) tx.rollback();
            logger.error("addTransactionProduct() " +
                    "– błąd podczas zapisu {}", tp, e);
            throw e;
        } finally {
            em.close();
        }
    ...
    public TransactionProduct findById(
        ...
        logger.debug("findById() – start, txId={}, prodId={}",
                transactionId, productId);
        EntityManager em = EMFProvider.get().createEntityManager();
        try {
            TransactionProductId id
                    = new TransactionProductId(transactionId, productId);
    ...
    public List<TransactionProduct> getByTransaction(int transactionId) {
        logger.debug("getByTransaction() – start, txId={}",
                transactionId);
        EntityManager em = EMFProvider.get().createEntityManager();
        try {
            TypedQuery<TransactionProduct> q = em.createQuery(
                    "SELECT tp FROM TransactionProduct tp " +
                            "WHERE tp.transaction.id = :txId",
                    TransactionProduct.class);
            q.setParameter("txId", transactionId);
            List<TransactionProduct> list = q.getResultList();
            logger.info("getByTransaction() – zwrócono {} pozycji",
                    list.size());
            return list;
        } catch (Exception e) {
            logger.error("getByTransaction() " +
    ...
    public List<TransactionProduct> getByProduct(int productId) {
        ...
        try {
            TypedQuery<TransactionProduct> q = em.createQuery(
                    "SELECT tp FROM TransactionProduct tp " +
                            "WHERE tp.product.id = :prodId",
                    TransactionProduct.class);
            q.setParameter("prodId", productId);
            List<TransactionProduct> list = q.getResultList();
            logger.info("getByProduct() – zwrócono {} pozycji",
                    list.size());
            return list;
        } catch (Exception e) {
    ...
    public List<TransactionProduct> getAllTransactionProducts() {
        ...
        try {
            List<TransactionProduct> list = em.createQuery(
                            ...
                            TransactionProduct.class)
                    .getResultList();
            logger.info("getAllTransactionProducts() " +
                    "– zwrócono {} pozycji", list.size());
            return list;
        } catch (Exception e) {
            logger.error("getAllTransactionProducts() " +
                    "– błąd podczas zapytania", e);
            return List.of();
        } finally {
            em.close();
    ...
    public void updateTransactionProduct(TransactionProduct tp) {
        ...
        try {
            ...
            em.merge(tp);
            tx.commit();
            logger.info("updateTransactionProduct() " +
                    "– zaktualizowano: {}", tp);
        } catch (Exception e) {
            logger.error("updateTransactionProduct() " +
    ...
    public void removeTransactionProduct(TransactionProduct tp) {
        ...
        try {
            tx.begin();
            TransactionProduct managed
                    = em.find(TransactionProduct.class, tp.getId());
            if (managed != null) {
                em.remove(managed);
                logger.info("removeTransactionProduct() " +
                        "– usunięto: {}", tp);
            } else {
                logger.warn("removeTransactionProduct() " +
                        "– nie znaleziono: {}", tp);
            }
            tx.commit();
        } catch (Exception e) {
            logger.error("removeTransactionProduct() " +
